...
Whetstone's investigation and spec-drafting skill. Takes one failure mode id, builds a deep understanding of what's failing and why, flags anything that requires SME input or external change, and produces a `SPEC.md` for human review. Stops at a hard approval gate — no code is touched until the user signs off on the spec.

This skill is **read-only with respect to the agent codebase.** It reads traces, reads source code, and writes `whetstone/failure_modes/<id>/SPEC.md` plus status updates to `failure_modes.json`. It never edits agent code, never commits, never pushes.

## Inputs

- **failure_mode_id** (required) — the `id` of the failure mode to research, e.g. `fm_2026_04_hallucinated_action`. If the user didn't provide one, run `npx whetstone status` to list open failure modes and ask which to work on.
...
2. **The catalogue is valid.** Run `npx whetstone validate`. If it exits non-zero, show the output and stop — a broken catalogue must be fixed before further analysis.
3. **The failure mode exists.** Run `npx whetstone fm get <id>`. If "not found", stop. Print the returned record so both you and the user can see the current state.
4. **The failure mode is researchable.** Status must be one of: `discovered`, `triaged`, `investigating`, `regressed`. If it's already `spec_drafted`, `fix_approved`, `fix_in_progress`, `verifying`, `verified`, or `hardened`, tell the user — a spec may already exist at `whetstone/failure_modes/<id>/SPEC.md` — and ask whether they want to revise it or continue to implementation. If `wont_fix`, `closed`, or `duplicate_of:…`, stop.
5. **Read the project config.** Load `whetstone/whetstone.config.md`. Quote the **Hard rules** section back to the user before doing any work so they're always visible in the transcript.
...
```bash
cat whetstone/traces/$(jq -r .originalTraceFile <<< '<trace-json-line>')
```
...
Update `failure_modes.json`: set `status = "investigating"`, `lastUpdated = <UTC ISO>`. Run `npx whetstone validate`. Self-correct if it fails.

---

## Step 2 — Read the agent source

Use the "Agent under test" section of `whetstone.config.md` to locate the repo root.
...
```bash
mkdir -p whetstone/failure_modes/<id>
```

Write `whetstone/failure_modes/<id>/SPEC.md`:
...
Update `failure_modes.json`: set `status = "spec_drafted"`, `lastUpdated = <UTC ISO>`.
...
When approved:
- Update `failure_modes.json`: set `status = "fix_approved"`, `lastUpdated = <UTC ISO>`.
- Run `npx whetstone validate`. Self-correct if it fails.
- Print: "Spec approved. Run the `implement-failure-mode` skill with `<id>` to begin implementation."

---

## Hard rules

- **This skill never edits agent code.** Read-only with respect to everything except `whetstone/failure_modes/<id>/SPEC.md` and `failure_modes.json`.
- **Spec approval is a hard gate.** Do not mark status `fix_approved` until the user explicitly approves.
- **Write `failure_modes.json` and run `whetstone validate` after every change.** Self-correct if it fails.
...
- **Honor all hard rules from `whetstone.config.md`.** Quote them at the start of the transcript.

## Output contract

After this skill runs to completion:

- `whetstone/failure_modes/<id>/SPEC.md` exists with all required sections filled in.
- `failure_modes.json` has `status = "fix_approved"` for this failure mode (set only after explicit user approval).
