...
Whetstone's implementation and verification skill. Takes one failure mode id with an approved `SPEC.md`, implements the fix in the agent's working tree, runs sanity checks, and verifies the fix by invoking the model with test inputs derived from the failure mode's cohort. Stops after verification and leaves the working tree ready for the user to review.
...
1. **Working tree is in a Whetstone project.** `whetstone/whetstone.config.md`, `whetstone/failure_modes.json`, and `whetstone/traces/` all exist.
2. **The catalogue is valid.** Run `npx whetstone validate`. If non-zero, show the output and stop.
3. **The failure mode exists.** Run `npx whetstone fm get <id>`. Print the record.
4. **The spec exists.** `whetstone/failure_modes/<id>/SPEC.md` must exist and contain all required sections (What's failing, Root cause, Proposed fix, Acceptance criteria, Test plan). If missing, tell the user to run `research-failure-mode` first.
...
6. **Read the project config.** Load `whetstone/whetstone.config.md`. Quote the **Hard rules** section back to the user before doing any work.
7. **Check for a model test command.** Look for the `## Model test command` section in `whetstone.config.md`. This section must contain a `Command:` line with the CLI to use for live verification. See the "Verification" section below for how it is used.
   ...
   **If the section is absent or has no `Command:` line**, say:
   ...
   > If you want live model testing, add a `## Model test command` section to `whetstone/whetstone.config.md` with a `Command:` line, then re-run this skill.
...
Read `whetstone/failure_modes/<id>/SPEC.md` in full. Pay special attention to:
...
Write `whetstone/failure_modes/<id>/PLAN.md`:
...
## Sanity checks (from whetstone.config.md)
...
Update `failure_modes.json`: set `status = "fix_in_progress"`, `lastUpdated = <UTC ISO>`. Run `npx whetstone validate`. Self-correct if it fails.

### Branching

If `whetstone.config.md` specifies a branching rule (e.g. "Always work on `whetstone/<failure_mode_id>` branches"), create the branch before touching code:
...
3. Run the project's sanity checks (from `whetstone.config.md` "Sanity checks" section)
4. If any check fails, fix it before moving to the next step. Do not accumulate failing checks.

**Constraints:**
- Honor every hard rule from `whetstone.config.md`. If a rule says "never modify `agent/src/payments/**` without a human in the loop", stop before touching those files and ask.
...
Update `failure_modes.json`: set `status = "verifying"`, `lastUpdated = <UTC ISO>`. Run `npx whetstone validate`. Self-correct if it fails.

### Build the test cohort

```bash
jq -c --arg fmid "<id>" 'select(.failureModeIds | index($fmid) != null)' \
  whetstone/traces/*.jsonl
```
...
Read the `Command:` line from the `## Model test command` section of `whetstone.config.md`.
...
jq -r --arg fmid "<id>" \
  ...
  whetstone/traces/*.jsonl > /tmp/cohort-<id>.txt
...
- The status is set to `verified (sanity-checks only)` — represent this in `failure_modes.json` as `status = "verifying"` with a note in the FM description that live model tests are still pending.
- Print clearly: "Sanity checks passed but the fix has not been verified against the live model. Add a `## Model test command` to `whetstone.config.md` and re-run this skill (or use `verify-failure-mode`) to complete verification."
- Stop. Do not set status to `verified`.

### Update status

**On pass:**
- Set `status = "verified"`, `lastUpdated = <UTC ISO>` in `failure_modes.json`.
- Run `npx whetstone validate`. Self-correct if it fails.
...
- Next steps:
  ...
  - Run `npx whetstone status` to see other open failure modes.
...
- **Honor all hard rules from `whetstone.config.md`.** Quote them at the start of the transcript. Stop and ask if a rule blocks a necessary change.
...
- **Write `failure_modes.json` and run `whetstone validate` after every status change.** Self-correct if it fails.
...
- `failure_modes.json` has `status = "verified"` (or `"verifying"` if live model tests were skipped with user consent).
- `npx whetstone validate` passes.
