...
Whetstone's implementation and verification skill. Operates on a single agent at a time. Takes one failure mode id with an approved `SPEC.md`, implements the fix in the agent's working tree, runs sanity checks, and verifies the fix by invoking the model with test inputs derived from the failure mode's cohort. Stops after verification and leaves the working tree ready for the user to review.
...
2. **The catalogue is valid.** Run `npx whetstone validate --agent <agent_name>`. If non-zero, show the output and stop.
3. **The failure mode exists.** Run `npx whetstone fm get <id> --agent <agent_name>`. Print the record.
4. **The spec exists.** `whetstone/<agent_name>/failure_modes/<id>/SPEC.md` must exist and contain all required sections (What's failing, Root cause, Proposed fix, Acceptance criteria, Test plan). If missing, tell the user to run `research-failure-mode` first.
...
6. **Read the project config.** Load `whetstone/<agent_name>/whetstone.config.md`. Quote the **Hard rules** section back to the user before doing any work.
7. **Check for a model test command.** Look for the `## Model test command` section in `whetstone/<agent_name>/whetstone.config.md`. This section must contain a `Command:` line with the CLI to use for live verification. See the "Verification" section below for how it is used.
   ...
   **If the section is absent or has no `Command:` line**, say:
   ...
   > If you want live model testing, add a `## Model test command` section to `whetstone/<agent_name>/whetstone.config.md` with a `Command:` line, then re-run this skill.
...
Read `whetstone/<agent_name>/failure_modes/<id>/SPEC.md` in full. Pay special attention to:
...
Write `whetstone/<agent_name>/failure_modes/<id>/PLAN.md`:
...
## Sanity checks (from whetstone/<agent_name>/whetstone.config.md)
...
Update `whetstone/<agent_name>/failure_modes.json`: set `status = "fix_in_progress"`, `lastUpdated = <UTC ISO>`. Run `npx whetstone validate --agent <agent_name>`. Self-correct if it fails.

### Branching

If `whetstone/<agent_name>/whetstone.config.md` specifies a branching rule (e.g. "Always work on `whetstone/<failure_mode_id>` branches"), create the branch before touching code. The branch name template stays as the user configured it — `whetstone/<id>` here is a *git branch name*, not a file path:

```bash
git checkout -b whetstone/<id>
```
...
3. Run the project's sanity checks (from `whetstone/<agent_name>/whetstone.config.md` "Sanity checks" section)
4. If any check fails, fix it before moving to the next step. Do not accumulate failing checks.

**Constraints:**
- Honor every hard rule from `whetstone/<agent_name>/whetstone.config.md`. If a rule says "never modify `agent/src/payments/**` without a human in the loop", stop before touching those files and ask.
...
Update `whetstone/<agent_name>/failure_modes.json`: set `status = "verifying"`, `lastUpdated = <UTC ISO>`. Run `npx whetstone validate --agent <agent_name>`. Self-correct if it fails.
...
Read the `Command:` line from the `## Model test command` section of `whetstone/<agent_name>/whetstone.config.md`.
...
jq -r --arg fmid "<id>" \
  ...
  whetstone/<agent_name>/traces/*.jsonl > /tmp/cohort-<id>.txt
...
- The status is set to `verified (sanity-checks only)` — represent this in `whetstone/<agent_name>/failure_modes.json` as `status = "verifying"` with a note in the FM description that live model tests are still pending.
- Print clearly: "Sanity checks passed but the fix has not been verified against the live model. Add a `## Model test command` to `whetstone/<agent_name>/whetstone.config.md` and re-run this skill (or use `verify-failure-mode`) to complete verification."
- Stop. Do not set status to `verified`.

### Update status

**On pass:**
- Set `status = "verified"`, `lastUpdated = <UTC ISO>` in `whetstone/<agent_name>/failure_modes.json`.
...
- Next steps:
  ...
  - Run `npx whetstone status --agent <agent_name>` to see other open failure modes for this agent.
...
- **Honor all hard rules from `whetstone/<agent_name>/whetstone.config.md`.** Quote them at the start of the transcript. Stop and ask if a rule blocks a necessary change.
- **One agent per invocation.** Never read or write under `whetstone/<other-agent>/`. The agent name scopes every Whetstone-side read and write this skill performs.
...
- **Write `failure_modes.json` and run `whetstone validate --agent <agent_name>` after every status change.** Self-correct if it fails.
...
- `whetstone/<agent_name>/failure_modes.json` has `status = "verified"` (or `"verifying"` if live model tests were skipped with user consent).
