...
Inserted an agent-name segment between `tracebound/` and the rest of the tree at every read/write site. New file [src/commands/agent-root.ts](../../src/commands/agent-root.ts) absorbs path resolution, name validation (`^[a-z0-9][a-z0-9_-]*$`), and existence checks; the five existing commands now delegate to it. New `tracebound agents` command lists every subdirectory of `tracebound/` that contains a `tracebound.config.md`. `init` takes a positional agent name; every other command grows a required `--agent`/`-a` flag. All four skills updated to thread `agent_name` through inputs, paths, and CLI invocations. Tests parameterised with a fixture agent and grew multi-agent isolation cases. 88 tests pass; built binary smoke-tested end-to-end.
...
- **Tests had a hidden coupling to the old "missing tracebound/" error path.** The original `validate.test.ts` test "missing tracebound/ directory reports WHET_STRUCT_MISSING and stops" relied on `runValidate` returning a one-issue report when `tracebound/` was absent. Under the new design `resolveAgentRootForRead` throws *before* the validator runs — that test became "missing agent directory rejects with 'no such agent'" via `assert.rejects`. A handful of similar tests in `status.test.ts` and `get.test.ts` shifted from "throws with `/tracebound\/ directory not found/`" to "throws with `/no such agent/`", which is more accurate to the new model.
...
- **`tracebound agents` exits 0 with empty output when `tracebound/` is absent**, matching AC 10. Treats absence as "zero agents configured" rather than as an IO error. This means a fresh repo that's never been initialised gets an empty answer rather than an exit-2 error — which is the natural reading of "list the agents in this repo: none".
...
- **The `git checkout -b tracebound/<id>` example in `implement-failure-mode/SKILL.md` was left untouched** — it's a git branch name (the user's branching convention), not a path. Added a one-line note in the skill explaining this so future readers don't try to "fix" it.
...
- **`templates/tracebound.config.md` references an `ingest-traces` skill that does not exist** in the repo (only `analyze-traces`, `research-failure-mode`, `implement-failure-mode`, `create-adapter` are in `skills/`). Pre-existing typo in the template; not changed under this spec.
